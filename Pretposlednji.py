...
def write_register(bus, address, reg, value, retries=5):
    """Write a single byte to a register on the Si5351A with retries"""
    for attempt in range(retries):
        try:
            bus.write_byte_data(address, reg, value)
            # Skip verification for Reg 177 (PLLA_RST is self-clearing) and Reg 1 (sticky interrupts may not clear due to persistent conditions in Si5351A)
            if reg == 0xB1 or reg == 1:
                return
            # Verify write for other registers
...
def initialize(bus, address):
    ...
    - Wait for device readiness (SYS_INIT=0)
    - Clear sticky interrupts
    - Disable spread spectrum
    - Perform full device reset with verification
    - Set crystal load capacitance
    - Power down unused outputs (CLK3-7 for 10-MSOP)
    - Set PLLA to 800 MHz
    - Configure fanout and PLL multiplier registers
    - Verify PLL configuration
    - Reset PLLA with verification
    - Wait for PLL lock with enhanced checks
    """
    print("Starting Si5351A initialization...")

    # Wait for device readiness (SYS_INIT=0)
    print("Waiting for Si5351A to complete system initialization (SYS_INIT=0)...")
    for _ in range(100):  # Timeout after ~5s
        try:
            status = read_register(bus, address, 0x00)
            if (status & 0x80) == 0:  # SYS_INIT (bit 7) = 0
                print(f"Device ready. Status Reg 0 = 0x{status:02X}")
                break
            time.sleep(0.05)
        except OSError:
            time.sleep(0.05)  # Retry on I2C error
    else:
        print("Timeout: Si5351A initialization did not complete (SYS_INIT stuck at 1). Check power/connections.")
        return

    # Mask LOS_CLKIN interrupt (not applicable for Si5351A, but may help with persistent status)
    write_register(bus, address, 2, 0x10)  # Set LOS_CLKIN_MASK=1 (bit 4)

    # Clear sticky interrupts
    write_register(bus, address, 1, 0x00)  # Clear all sticky bits (LOL_A/B, LOS, etc.)

    # Disable spread spectrum to avoid conflicts
    write_register(bus, address, 149, 0x00)  # SSC_EN=0 (bit 7=0)
    ...
    # Power down unused outputs (CLK3-7) for 10-MSOP to conserve power
    write_register(bus, address, 19, 0x80)  # CLK3_PDN=1
    write_register(bus, address, 20, 0x80)  # CLK4_PDN=1
    write_register(bus, address, 21, 0x80)  # CLK5_PDN=1
    write_register(bus, address, 22, 0xC0)  # CLK6_PDN=1 (bit7), FBA_INT=1 (bit6)
    write_register(bus, address, 23, 0x80)  # CLK7_PDN=1

    # Route crystal input to PLLA
    write_register(bus, address, 0x0F, 0x00)

    # Enable fanout for crystal input (set to 0x50: XO=1, MS=1, CLKIN=0 for Si5351A)
    write_register(bus, address, 0xBB, 0x50)
    ...
    # Verify PLLA configuration
    expected_vals = [0x00, 0x01, 0x00, 0x0E, 0x00, 0x00, 0x00, 0x00]  # For 32x integer
    ...
    # Verify crystal load and spread spectrum
    crystal_load = read_register(bus, address, 0xB7)
    if crystal_load != 0xD2:
        print(f"Warning: Crystal load capacitance (Reg 183) = 0x{crystal_load:02X} (expected 0xD2 for 10pF)")
    ss_en = read_register(bus, address, 149)
    if ss_en != 0x00:
        print(f"Warning: Spread spectrum enabled (Reg 149 = 0x{ss_en:02X}). Disabling to ensure PLL stability.")
        write_register(bus, address, 149, 0x00)

    # Reset PLLA with verification
    try:
        write_register(bus, address, 0xB1, 0x20)  # PLLA_RST=1 (self-clearing)
        time.sleep(0.05)  # Longer delay for reset
    ...
    # Wait for PLLA lock (check SYS_INIT=0 and LOL_A=0)
    for _ in range(50):
        status = read_register(bus, address, 0x00)
        if (status & 0xA0) == 0:  # SYS_INIT=0 (bit7), LOL_A=0 (bit5)
            print(f"PLLA locked successfully. Status Reg 0 = 0x{status:02X}")
            break
        time.sleep(0.05)
    else:
        print(f"PLLA failed to lock. Status Reg 0 = 0x{status:02X}")
        print("Possible causes: Incorrect crystal frequency, wrong load capacitance, or spread spectrum enabled.")
        print(f"Current crystal settings: Reg 183 = 0x{read_register(bus, address, 0xB7):02X} (expected 0xD2 for 10pF)")
        print(f"Spread spectrum: Reg 149 = 0x{read_register(bus, address, 149):02X} (expected 0x00)")
        return

    # Verify PLLA registers (post-reset)
...
def set_frequency(bus, address, clk_num, freq):
    ...
    if 150000000 < freq <= 200000000:
        ...
        c = 1048575
        if b == 0:
            c = 1  # Use c=1 for integer mode to match PLL setup
    ...
    try:
        ...
        if (status & 0x20) != 0 or (status & 0x80) != 0:
            ...
            print("Possible causes: Incorrect Multisynth settings or PLL instability.")
        else:
    ...
    while True:
        ...
        if cmd[0] == "exit":
            ...
            try:
                clk = int(cmd[1])
                freq = int(cmd[2])
                set_frequency(bus, address, clk, freq)  # Removed hardcoded *4
            except ValueError:
