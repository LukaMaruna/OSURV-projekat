...
def write_register(bus, address, reg, value):
    ...
    try:
        ...
        if read_back != value:
            print(f"I2C write verification failed for Reg {reg}: wrote 0x{value:02X}, read 0x{read_back:02X}")
            raise OSError(f"Write verification failed for Reg {reg}")
    except OSError as e:
        print(f"I2C write error to Reg {reg}: {e}")
        raise
...
def read_register(bus, address, reg):
    ...
    try:
        ...
        print(f"I2C read error from Reg {reg}: {e}")
        raise
...
def initialize(bus, address):
    ...
    write_register(bus, address, 0xB7, 0xD2)  # Reg 183 = 0xD2 (10pF)

    # Route crystal input to PLLA
    write_register(bus, address, 0x0F, 0x00)  # Reg 15 = 0x00 (XTAL to PLLA)

    # Enable fanout for crystal input
    write_register(bus, address, 0xBB, 0xD0)  # Reg 187 = 0xD0
    ...
    write_register(bus, address, 22, 0x40)  # FBA_INT=1

    # Reset PLLA
    write_register(bus, address, 0xB1, 0x20)
    time.sleep(0.01)

    # Wait for PLLA lock (check LOL_A, bit 5 of Reg 0)
    for _ in range(10):
        status = read_register(bus, address, 0x00)
        if (status & 0x20) == 0 and (status & 0x80) == 0:  # LOL_A=0, SYS_INIT=0
            break
        time.sleep(0.01)
    else:
