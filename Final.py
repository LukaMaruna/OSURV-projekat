...
def write_register(bus, address, reg, value, retries=5, skip_verify=False):
    ...
    for attempt in range(retries):
        try:
            ...
            # Skip verification for specific registers (e.g., self-clearing or problematic)
...
def initialize(bus, address):
    """
    Initialize the Si5351A with enhanced diagnostics:
    - Wait for device readiness (SYS_INIT=0)
    - Check device revision
    - Clear sticky interrupts and mask LOS_XTAL
    ...
    # Mask LOS_XTAL and LOS_CLKIN interrupts to prevent issues
    write_register(bus, address, 2, 0x18)  # LOS_XTAL_MASK=1 (bit 3), LOS_CLKIN_MASK=1 (bit 4)

    # Clear sticky interrupts
    write_register(bus, address, 1, 0x00, skip_verify=True)  # Clear all sticky bits

    # Disable spread spectrum
    write_register(bus, address, 149, 0x00)  # SSC_EN=0
    ...
    write_register(bus, address, 22, 0xC0)  # CLK6_PDN=1, FBA_INT=1
    write_register(bus, address, 23, 0x80)  # CLK7_PDN=1

    # Route crystal input to PLLA
    write_register(bus, address, 0x0F, 0x00)  # PLLA_SRC=0 (XTAL)

    # Enable fanout for crystal input (set to 0x50: XO=1, MS=1, CLKIN=0)
    ...
    # Reset PLLA with verification
    try:
        write_register(bus, address, 0xB1, 0x20, skip_verify=True)  # PLLA_RST=1
        time.sleep(0.1)  # Increased delay for reset
    ...
    # Wait for PLLA lock with retries
    for attempt in range(3):
        ...
        # Retry crystal initialization
        write_register(bus, address, 0xB7, 0xD2)  # Reset crystal load
        write_register(bus, address, 0xB1, 0x20, skip_verify=True)  # Reset PLLA
        time.sleep(0.1)
...
def set_frequency(bus, address, clk_num, freq):
    """
    Set output frequency on CLK0-CLK2 with enhanced verification.
    """
    ...
    # Set control register: enable output, integer mode, PLLA source, Multisynth, 8mA drive
